...
export async function changeHypeSquadHouse(houseId: number) {
    try {
        ...
        if (houseId === 0) {
            await RestAPI.del({ url: "/hypesquad/online", body: {} });
        } else {
        ...
        showToast(`Erreur HypeSquad: ${status ? `Status ${status}` : (err?.message || err)}`, Toasts.Type.FAILURE);
    }
}
...
function injectHypeSquadChangerIntoNativePanel() {
    ...
    if (editingPanel && !document.getElementById("vencord-hypesquad-changer-modal-container")) {
        const scroller = editingPanel.querySelector("[class*='content__523e1'], [class*='scroller_'], [class*='scrollRegion_'] > div");
        if (scroller) {
            const container = document.createElement("div");
            container.id = "vencord-hypesquad-changer-modal-container";
    ...
    if (settingsView && !document.getElementById("vencord-hypesquad-changer-container")) {
        ...
        if (isProfileTab) {
            ...
            if (contentColumn) {
                ...
                if (!targetParent.querySelector("#vencord-hypesquad-changer-container")) {
                    const container = document.createElement("div");
                    container.id = "vencord-hypesquad-changer-container";
...
function stopDomObserver() {
    ...
    for (const id of ["vencord-hypesquad-changer-container", "vencord-hypesquad-changer-modal-container"]) {
