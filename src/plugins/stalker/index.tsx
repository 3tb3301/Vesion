...
import { ChannelStore, GuildStore, Menu, PresenceStore, RelationshipStore, useEffect, UserProfileStore, UserStore, useState } from "@webpack/common";
...
export default definePlugin({
    ...
    flux: {
        ...
        VOICE_STATE_UPDATE(payload: any) {
            try {
                ...
                const newChannelId = payload?.channelId ?? payload?.channel_id ?? null;
                const oldChannelId = payload?.oldChannelId ?? payload?.old_channel_id ?? null;
                const guildId = payload?.guildId ?? payload?.guild_id ?? null;

                let voiceAction: "join" | "leave" | "move" | null = null;
                ...
                const channel = activeChannelId ? ChannelStore.getChannel(activeChannelId) : null;
                const channelName = channel?.name ?? activeChannelId ?? "Unknown Channel";
                const guild = guildId ? GuildStore.getGuild(guildId) : null;
                const guildName = guild?.name ?? null;

                const snapshot = lastKnownUsers.get(userId);
                const avatarUrl = snapshot?.avatar
                    ? `https://cdn.discordapp.com/avatars/${userId}/${snapshot.avatar}.png?size=64`
                    : undefined;
                ...
                addPresenceLog({
                    ...
                    type: "voice",
                    voiceAction,
                    voiceChannelId: activeChannelId ?? undefined,
                    voiceChannelName: channelName,
                    activitySummary: `voice:${voiceAction}:${channelName}`
                } as any);

                if (voiceAction === "join" && userConfig.notifyVoiceJoin !== false) {
                    try {
                        showNotification({
                            title: `${user.username} joined a voice channel`,
                            body: guildName
                ...
                if (voiceAction === "leave" && userConfig.notifyVoiceLeave !== false) {
                    try {
                        showNotification({
                            title: `${user.username} left a voice channel`,
                            body: guildName
                                ? `#${channelName} in ${guildName}`
                                : `#${channelName}`,
                            icon: avatarUrl
                ...
                if (voiceAction === "move") {
                    ...
                    const oldChannelName = oldChannel?.name ?? oldChannelId ?? "Unknown";
                    if (userConfig.notifyVoiceJoin !== false) {
                        try {
                            showNotification({
                                title: `${user.username} moved voice channels`,
                                body: guildName
                                    ? `${oldChannelName} -> ${channelName} in ${guildName}`
                ...
                logger.error("Error in VOICE_STATE_UPDATE handler", e);
            }
        },

        RELATIONSHIP_ADD(payload: any) {
            try {
                const relationship = payload?.relationship ?? payload;
                const addedUserId = relationship?.id ?? relationship?.user?.id;
                const addedUser = relationship?.user ?? (addedUserId ? UserStore.getUser(addedUserId) : null);
                const relationshipType = relationship?.type;

                if (!addedUserId) return;

                if (relationshipType !== 1) return;

                const whitelisted = getWhitelistedIds();


                const myFriends = new Set<string>(
                    (RelationshipStore?.getFriendIDs?.() ?? []) as string[]
                );


                if (whitelisted.includes(addedUserId)) {
                    const stalkedUser = addedUser ?? UserStore.getUser(addedUserId);
                    const userConfig = getUserConfig(addedUserId);

                    if (userConfig.notifyMutualFriend !== false) {
                        const snapshot = lastKnownUsers.get(addedUserId);
                        const avatarUrl = snapshot?.avatar
                            ? `https://cdn.discordapp.com/avatars/${addedUserId}/${snapshot.avatar}.png?size=64`
                            : undefined;

                        try {
                            showNotification({
                                title: `You are now friends with ${stalkedUser?.username ?? addedUserId}`,
                                body: "You have become mutual friends!",
                                icon: avatarUrl
                            });
                        } catch (e) { /* ignore */ }
                    }
                    ...
                    addPresenceLog({
                        userId: addedUserId,
                        username: stalkedUser?.username ?? addedUserId,
                        discriminator: stalkedUser?.discriminator,
                        timestamp: Date.now(),
                        previousStatus: undefined,
                        currentStatus: PresenceStore.getStatus(addedUserId) ?? null,
                        type: "mutual_friend",
                        mutualFriendId: addedUserId,
                        mutualFriendUsername: stalkedUser?.username ?? addedUserId,
                        activitySummary: "mutual_friend:added"
                    } as any);

                    return;
                }

                if (myFriends.has(addedUserId)) {
                    for (const stalkedId of whitelisted) {
                        if (stalkedId === UserStore.getCurrentUser()?.id) continue;

                        const stalkedUserFriends = (RelationshipStore as any)?.getRelationships?.() ?? {};
                        const isFriendWithAdded = stalkedUserFriends[addedUserId] === 1;

                        if (isFriendWithAdded) {
                            const stalkedUser = UserStore.getUser(stalkedId);
                            const addedFriendUser = addedUser ?? UserStore.getUser(addedUserId);
                            const userConfig = getUserConfig(stalkedId);

                            if (userConfig.notifyMutualFriend !== false) {
                                const snapshot = lastKnownUsers.get(stalkedId);
                                const avatarUrl = snapshot?.avatar
                                    ? `https://cdn.discordapp.com/avatars/${stalkedId}/${snapshot.avatar}.png?size=64`
                                    : undefined;

                                try {
                                    showNotification({
                                        title: `${stalkedUser?.username ?? stalkedId} added your friend`,
                                        body: `They are now friends with ${addedFriendUser?.username ?? addedUserId}`,
                                        icon: avatarUrl
                                    });
                                } catch (e) { /* ignore */ }
                            }
                            ...
                            addPresenceLog({
                                userId: stalkedId,
                                username: stalkedUser?.username ?? stalkedId,
                                discriminator: stalkedUser?.discriminator,
                                timestamp: Date.now(),
                                previousStatus: undefined,
                                currentStatus: PresenceStore.getStatus(stalkedId) ?? null,
                                type: "mutual_friend",
                                mutualFriendId: addedUserId,
                                mutualFriendUsername: addedFriendUser?.username ?? addedUserId,
                                activitySummary: `mutual_friend:${addedUserId}`
                            } as any);
                ...
                logger.error("Error in RELATIONSHIP_ADD handler", e);
            }
        },
